...
<div class="box box-info">
	...
	<div class="box-header with-border">
		<div class=" col-md-2">
			<select name="search" class="form-control" id="search" onChange="getOutletById($(this).val());">
		...
		<button type="button" class="btn btn-primary btn-sm pull-right" id="btn-add" >
			...
			<button style="margin-right: 10px" type="button" class="btn btn-primary btn-sm pull-right" id="btn-export">
...
<div class="modal" id="modal-form2">
	<div class="modal-dialog">
		<div class="box box-success">
			<div class="box-header">
				<h3 class="box-title" id="modal-title2">Form Input</h3>
			</div>
			<div class="box-body" id="modal-data2">
			...
			success : function(result){
				...
				$.each(result, function(index, item){
					var dataRow ='<tr>'+
						...
						'<td>'+ item.status +'</td>'+
						'<td class="col-md-1">'+
							'<button type="button" class="btn btn-view btn-warning btn-xs" value="'+ item.id +'"><i class="fa fa-edit"></i> View</button> '+
						'</td>'+
						'</tr>';
...
function loadOutlet($form, $selected){
	$.ajax({
		...
		success: function(result){
			$("#search").empty();
			$("#search").append('<option value="">To Outlet</option>');
			$.each(result, function(index, outlet){
				if($selected == outlet.id){
					$("#search").append('<option value="'+ outlet.id +'" selected="selected">'+ outlet.name +'</option>');
				} else {
					$("#search").append('<option value="'+ outlet.id +'">'+ outlet.name +'</option>');
				}
			});
		}
	});
...
function getSearch(){
	...
	$.ajax({
		url: '${contextName}/api/ts/search/'+dataId,
		type:'get',
		dataType: 'json',
		success: function(result){
			//kosong data di table
